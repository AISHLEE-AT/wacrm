...
let content = fs.readFileSync('apps/web/src/app/(aishlee)/teacho/page.tsx', 'utf-8');

// 1. Fix the localStorage detach invocation bug
content = content.replace(
  `  const [activeCourse, setActiveCourse] = useState<any>(() => {
    try {
      if (typeof window !== "undefined") {
        const stored = localStorage.getItem("teacho_active_course");
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        const stored = localStorage.getItem("teacho_active_course");
        if (stored) setActiveCourse(JSON.parse(stored));
      } catch (e) {}
    }
  }, []);`
);
...
content = content.replace(
  `  const [courseContent, setCourseContent] = useState<any[]>(() => {
    try {
      if (typeof window !== "undefined") {
        const stored = localStorage.getItem("teacho_course_content");
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        const stored = localStorage.getItem("teacho_course_content");
        if (stored) setCourseContent(JSON.parse(stored));
      } catch (e) {}
    }
  }, []);`
);
...
if (parts.length < 2) {
    const partsLF = content.split('  return (\n    <div className="animate-fade-in-up bento-grid">');
    if (partsLF.length < 2) {
        console.error("Could not find split string for UI");
        process.exit(1);
    }
    content = partsLF[0];
} else {
...
fs.writeFileSync('apps/web/src/app/(aishlee)/teacho/page.tsx', content);
