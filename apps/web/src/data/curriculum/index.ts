...
export function resolveMasterCurriculumPlan(
  ...
  for (let tNum = 1; tNum <= taskCount; tNum++) {
    ...
    tasks.push({
      ...
      subtitle: item.subtopic ? `${item.chapterTitle} • ${item.subtopic}` : (item.overview || '').substring(0, 100),
      subject: item.subject || 'Core Subject',
      topic: item.topicTitle,
      subtopic: item.subtopic,
      rawSubject: item.subject || 'Core Subject',
      rawTopic: item.topicTitle,
      aiPrompt: (item as any).aiPrompt || `Explain "${item.topicTitle}" from ${item.subject || courseTitle} with key concepts, formulas, examples, and 3 exam tips in Tamil & English.`,
      keyFormula: item.keyFormulaOrLaw || (item as any).formula || '',
      learningObjective: item.overview || '',
