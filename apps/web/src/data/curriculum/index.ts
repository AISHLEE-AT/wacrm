...
export function resolveMasterCurriculumPlan(
  ...
  const isTamilMedium = courseTitle.includes('தமிழ்') || courseId.includes('-ta-');
  ...
  const icons = ['📜', '📐', '🔬', '🌍', '📚', '🎯'];
  const taskTypes: Array<DailySubjectTask['taskType']> = ['reading', 'practice', 'video', 'activity', 'test'];
