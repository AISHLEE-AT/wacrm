...
import { ExternalLink, RefreshCw, Tractor } from 'lucide-react';
import { createClient } from '@/lib/supabase/client';

export default function RentOPage() {
  const [sessionParams, setSessionParams] = useState('');
  const [iframeUrl, setIframeUrl] = useState('https://thamizhan.vercel.app/rento');
    ...
    async function syncSession() {
      try {
        ...
        if (session) {
          ...
          setIframeUrl(`https://thamizhan.vercel.app/rento${tokens}`);
        }
      } catch (err) {
      ...
      <div className="flex items-center justify-between bg-card/80 border border-white/10 px-4 py-2.5 rounded-2xl shadow-sm backdrop-blur-md shrink-0">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-sm sm:text-base font-bold text-foreground">
            RentO - Aishlee Technology Agri Machinery Rentals
          </h1>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              const base = 'https://thamizhan.vercel.app/rento';
