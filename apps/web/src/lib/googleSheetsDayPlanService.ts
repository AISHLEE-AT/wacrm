...
export const GoogleSheetsDayPlanService = {
  getSavedConfig(): GoogleSheetConfig | null {
  ...
  saveConfig(config: GoogleSheetConfig): void {
  ...
  getCachedDayPlans(): Record<string, GoogleSheetDayPlanItem> {
    if (typeof window === 'undefined') return {};
    try {
      const raw = localStorage.getItem(STORAGE_KEYS.DAY_PLANS);
  ...
  getDayPlan(courseId: string, dayNumber: number): GoogleSheetDayPlanItem | null {
    const all = this.getCachedDayPlans();
  ...
  async syncGoogleSheet(urlOrId: string, sheetName: string = 'Sheet1'): Promise<{
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem(STORAGE_KEYS.DAY_PLANS, JSON.stringify(planStore));
      }
      ...
      try {
        ...
        console.warn('[GoogleSheetsDayPlanService] Supabase sync warning:', sbErr);
      }
