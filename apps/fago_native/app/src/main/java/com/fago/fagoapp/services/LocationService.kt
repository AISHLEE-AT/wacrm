 ...
 * Fetches high-precision device GPS location and provides reverse geocoding.
 */
    ...
    suspend fun getAddressFromLatLng(latLng: LatLng): String = withContext(Dispatchers.IO) {
        try {
            ...
            if (!addresses.isNullOrEmpty()) {
                ...
                if (!line.isNullOrEmpty()) return@withContext line
                val locality = addr.locality ?: addr.subAdminArea ?: ""
                val adminArea = addr.adminArea ?: ""
                if (locality.isNotEmpty()) return@withContext "$locality, $adminArea"
            }
        } catch (e: Exception) {
